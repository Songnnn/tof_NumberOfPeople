...
##### fileRead.h中的全局变量
`int depth[320*240]={0}`<br>320\*240是视频帧的像素点个数，即videoWidth\*videoHight
`videoPath`<br>深度数据转出的伪彩视频所在地址
`framesFilePath`<br>伪彩视频帧所在地址
`depthDatasPath`<br>深度数据所在地址
`videoWidth`<br>像素点矩阵的宽
`videoHight`<br>像素点的高
##### getNumbers.h中的全局变量
`people`
